...
import {CALL_HELP_REQUEST, CALL_HELP_SUCCESS, CALL_HELP_FAILURE} from './types';
import {
  SUBMIT_TEST_REQUEST,
  SUBMIT_TEST_SUCCESS,
  SUBMIT_TEST_FAILURE,
...
import {
  ...
  CANDIDATE_DETAILS_CLEAR_SUCCESS,
} from './types';
import {
...
const _axios = () =>
  axios.create({
    baseURL: 'http://api.recruit.excellencetechnologies.in/',
    timeout: 20000,
...
export const getExamQuestions = data => async dispatch => {
  ...
  try {
    const res = await axios.get(
      `http://176.9.137.77:8081/exams/getQuestinsForCandidate/${data}`,
    );
...
export const verifyCandidate = data => async dispatch => {
  ...
  try {
    const res = await axios.post(
      `http://176.9.137.77:8081/exams/verify_candidate`,
      {...data},
    );
...
export const addNewCandidate = data => async dispatch => {
  ...
  try {
    const res = await axios.post(
      `http://176.9.137.77:8081/exams/addNewCandidate`,
      {...data},
    );
...
export const addCandidate = data => async dispatch => {
  ...
  try {
    const res = await _axios().post('exams/addCandidateWithBase64File', {
      ...data,
    });
...
export const callingHelp = (accessToken, fb_id, message) => async dispatch => {
  ...
  try {
    const res = await _axios().post(
      'exams/askHrForHelp?accessToken=${accessToken}',
      {
        fb_id,
        message,
      },
...
export const getCandidateUpdateProfileDetails = _id => async dispatch => {
  ...
  try {
    ...
    dispatch({
      type: CANDIDATE_UPDATE_PROFILE_DETAILS_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
    if (err.message == 'timeout of 10000ms exceeded') {
...
export const candidateValidationapi = (data, fcm_token) => async dispatch => {
  ...
  try {
    const res = await _axios().post('exam/candidateValidate', {
      email: data,
      device_id: fcm_token,
    });
